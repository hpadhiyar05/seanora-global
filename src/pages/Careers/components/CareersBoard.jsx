        ...
        <>
            ...
            <div className="relative z-20">
                <button
                    type="button"
                    onClick={onToggle}
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-[0.875rem] font-medium transition-all duration-300 ${
                        isActive
                            ? 'bg-[#1E5AA5] text-white shadow-md shadow-[#1E5AA5]/20'
                            : 'bg-transparent text-[#6B6B6B] border border-black/10 hover:border-black/20 hover:text-[#1B1D1E]'
                    }`}
                        ...
                        >
                            {options.map((option) => (
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-2.5 text-[0.875rem] transition-colors duration-150 hover:bg-[#EFF6FF] flex items-center justify-between gap-3 ${
                                        value === option ? 'text-[#1E5AA5] font-semibold' : 'text-[#6B6B6B]'
                                    }`}
                                >
                                    {option}
        ...
        >
            {hasFilters && (
                <button
                    type="button"
                    onClick={onClear}
                    className="px-6 py-3 rounded-full text-[0.875rem] font-semibold bg-[#1E5AA5] text-white hover:bg-[#0B1F3B] transition-colors duration-300"
                >
                    Clear all filters
                </button>
            )}
            <a
                href="mailto:info@seanoraglobal.com"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-[0.875rem] font-medium border border-black/10 text-[#1B1D1E] hover:border-black/25 transition-colors duration-300"
            >
                <Mail className="w-3.5 h-3.5" strokeWidth={1.5} />
                Send us your resume
            </a>
...
const CareersBoard = () => {
    ...
    const filteredJobs = jobs.filter((job) => {
        ...
        const jobLocation = (job?.location ?? '').toString();
        const selectedLocation = (activeLocation ?? '').toString();
        const matchLocation = selectedLocation === 'All Locations' || jobLocation.toLowerCase().includes(selectedLocation.toLowerCase());
        return matchCategory && matchType && matchLocation;
    });
        ...
        <section className="pt-32 pb-16 md:pt-40 md:pb-24 bg-white min-h-screen relative overflow-hidden">
            <div className="container mx-auto px-4 lg:px-12 max-w-5xl relative z-10">
                ...
                >
                    {/* Left: text */}
                    <div className="flex-1">
                        <span className="text-[0.875rem] font-semibold tracking-[0.22em] text-[#6B6B6B] uppercase mb-6 block">
                            Join Our Team
                        </span>
                        <AnimatedHeading
                            as="h1"
                            className="text-[3rem] md:text-[5rem] font-medium text-[#111827] mb-8 leading-[1.04] tracking-[-0.02em]"
                        >
                            <AnimatedText text="Be part of our " />
                            <AnimatedText text="mission" className="text-[#1B1D1E]/45" />
                        </AnimatedHeading>
                        <p className="text-[1.25rem] text-[#6B6B6B] font-light leading-[1.8] max-w-lg">
                ...
                >
                    ...
                    {CATEGORIES.map((category) => (
                        <button
                            ...
                            onClick={() => setActiveCategory(category)}
                            type="button"
                            className={`px-5 py-2.5 rounded-full text-[0.875rem] font-medium transition-all duration-300 ${
                                activeCategory === category
                                    ? 'bg-[#1E5AA5] text-white shadow-md shadow-[#1E5AA5]/20'
                                    : 'bg-transparent text-[#6B6B6B] border border-black/10 hover:border-black/20 hover:text-[#1B1D1E]'
                            }`}
                                ...
                                >
                                    <div className="group bg-white border border-black/8 hover:border-[#1E5AA5]/20 rounded-2xl p-6 md:p-8 transition-all duration-300 hover:shadow-[0_12px_40px_rgba(30,90,165,0.08)] relative overflow-hidden">
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                                            <div className="flex-1">
                                                <div className="flex flex-wrap items-center gap-3 mb-3">
                                                    <span className="text-[0.875rem] font-semibold text-white bg-[#1E5AA5] px-3 py-1 rounded-full tracking-[0.04em]">
                                                        {job.department}
                                                    </span>
                                                    {job.salary && (
                                                        <span className="text-[0.875rem] font-medium text-[#6B6B6B] bg-black/5 px-3 py-1 rounded-full">
                                                            {job.salary}
                                                        </span>
                                                    )}
                                                </div>

                                                <h3 className="text-[1.5rem] md:text-[1.75rem] font-medium text-[#111827] mb-3 leading-snug tracking-[-0.015em] group-hover:text-[#111827]/80 transition-colors duration-300">
                                                    {job.title}
                                                </h3>

                                                <p className="text-[1rem] text-[#6B6B6B] mb-5 font-light leading-[1.75] max-w-xl">
                                                    {job.description}
                                                </p>

                                                <div className="flex flex-wrap items-center gap-4">
                                                    <div className="inline-flex items-center gap-2 text-[0.875rem] font-medium text-[#6B6B6B]">
                                                        <MapPin className="w-3.5 h-3.5" strokeWidth={1.5} />
                                                        {job.location}
                                                    </div>
                                                    <div className="w-1 h-1 rounded-full bg-black/15" />
                                                    ...
                                                    {job.experience && (
                                                        <>
                                                            <div className="w-1 h-1 rounded-full bg-black/15" />
                                                            <div className="text-[0.875rem] font-medium text-[#6B6B6B]">
                                                                {job.experience} exp.
                                                            </div>
                                                        </>
                                                    )}
                                            ...
                                            <Link
                                                ...
                                                aria-label={`View details for ${job.title}`}
                                            >
                                                <span className="text-[0.875rem] font-semibold pl-4 pr-2 whitespace-nowrap transition-transform duration-[400ms] ease-out group-hover/btn:translate-x-[36px]">
                                                    More Details
                                                </span>
                                                <div className="w-9 h-9 rounded-full bg-white text-[#1E5AA5] flex items-center justify-center shrink-0 transition-transform duration-[400ms] ease-out group-hover/btn:-translate-x-[128px]">
                                                    <ArrowUpRight className="w-4 h-4" />
